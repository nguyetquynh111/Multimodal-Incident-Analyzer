...
| T-012 | Add image OCR/object mapping | Zainab | Medium | Supported evidence maps to the image draft; available Roboflow bounding boxes appear in Visual Evidence; corner OCR keeps generic readable text after cleanup; image OCR text may populate final Location through `llm_summarizer.update_image_location(...)`; no detected objects may use `None`, empty OCR may use `N/A`, and Roboflow failure writes safe empty artifacts without crashing |
