...
| --- | --- |
| app.py | Streamlit UI, single-file Add Incident flow, session-local Combine Reports preview, Supabase dashboard, Manage Incidents CRUD, and CSV downloads |
...
| cloud_deployment/supabase_client.py | Create Supabase client and insert/query/update/delete user-confirmed incident rows; final returned rows use the nine-field table schema |
...
| SUPABASE_SERVICE_ROLE_KEY | Yes, unless an anon/compatibility key is provided | Preferred Supabase key for trusted hosted demos |
| SUPABASE_ANON_KEY | Yes, unless service role or compatibility key is provided | Supabase anon key for local/class demo setups |
| SUPABASE_KEY | Yes, unless one of the named keys above is provided | Backward-compatible Supabase key name used by `.env.example` and the GitHub deployment workflow |
...
For deployment, store Supabase values in the host's secret manager, GitHub Actions secrets, Streamlit secrets, or environment settings. The app bridges `st.secrets` into the same environment variable names when running under Streamlit. Do not commit credentials or raw evidence. The hosted app only needs to support the classroom workflow; production availability and emergency-service security certification are out of scope.
