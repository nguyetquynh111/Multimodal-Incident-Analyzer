...
OUTPUT_PATH = Path(__file__).resolve().parent / "output" / "audio_output.csv"
CHECKPOINT_EVERY = 10
...
def load_existing_results(output_path: Path) -> pd.DataFrame:
    if not output_path.exists() or output_path.stat().st_size == 0:
        return pd.DataFrame(columns=AUDIO_OUTPUT_COLUMNS)
    results = pd.read_csv(output_path)
    if list(results.columns) != AUDIO_OUTPUT_COLUMNS:
...
def save_results(results: pd.DataFrame, output_path: Path) -> None:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            results.reindex(columns=AUDIO_OUTPUT_COLUMNS).to_csv(temporary, index=False)
...
def main() -> int:
    ...
    completed = set(results["Call_ID"].dropna().astype(str))
    pending = [path for path in files if path.stem not in completed]
    ...
    try:
        for index, audio_path in enumerate(pending, start=1):
            try:
                extractor_frame = process_audio(audio_path, raise_on_transcription_error=True)
                row = extractor_frame.iloc[0]
                status = f"{row['raw_event']} / {row['raw_severity']}"
            except Exception as exc:
                failures += 1
                print(f"ERROR {audio_path.name}: {exc}", file=sys.stderr)
                extractor_frame = unknown_result(audio_path)
                status = "Unknown fallback"

            frame = to_audio_output(
                extractor_frame,
                audio_paths={audio_path.name: audio_path},
            )
